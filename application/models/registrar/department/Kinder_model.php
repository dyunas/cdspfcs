...
class Kinder_model extends CI_Model {
	...
	public function check_stud_lrn($lrn)
	{
		...
		$this->db->from('tbl_stud_info_kinder');
	...
	public function register_student()
	{
		...
		if ($this->db->insert('tbl_stud_info_kinder', $data))
		{
			$adtnl_data = array(
				"stud_lrn" 				=> $this->input->post('LRN'),
			...
			if ($this->db->insert('tbl_stud_adtnl_info_kinder', $adtnl_data))
			{
				$sub_docs = array(
					"stud_lrn" 		=> $this->input->post('LRN'),
				...
				if ($this->db->insert('tbl_stud_documents_kinder', $sub_docs))
				{
					$studid = ($this->input->post('LRN') != '') ? $this->input->post('LRN') : 'N/A';
					$logs = array(
						"emp_id" => $this->session->userdata('uniq_id'),
						"c_log" => "Registered student with LRN/Student ID of ".$studid,
	...
	public function update_student()
	{
		...
		if ($this->db->update('tbl_stud_info_kinder', $data))
		{
			$adtnl_data = array(
				"stud_lrn" 				=> $this->input->post('LRN'),
				"stud_grdns_name" => $this->input->post('grdns_name'),
			...
			if ($this->db->update('tbl_stud_adtnl_info_kinder', $adtnl_data))
			{				
				$sub_docs = array(
					"stud_lrn" 		=> $this->input->post('LRN'),
					"bCertPSA"		=> ($this->input->post('bCertPSA') != NULL) ? $this->input->post('bCertPSA') : 0,
				...
				$this->db->where('row_id', $this->input->post('row_id'));
				if ($this->db->update('tbl_stud_documents_kinder', $sub_docs))
				{
					$logs = array(
						"emp_id" => $this->session->userdata('uniq_id'),
	...
	public function update_student_admission_status()
	{
		...
		if ($this->db->update('tbl_stud_info_kinder'))
		{
			$logs = array(
				"emp_id" => $this->session->userdata('uniq_id'),
				"c_log" => "Updated record of student with LRN/Student ID of ".$this->input->post('LRN'),
	...
	public function get_kinder_table_data()
	{
		...
		$this->db->from('tbl_stud_info_kinder a');
		...
		if ($query->num_rows() > 0)
		{
			foreach ($query->result() as $row)
			{
				...
				$sbdata[] = $status;
				$sbdata[] = '<a href="kinder/view/'.$row->stud_lrn.'" class="btn btn-outline-primary btn-sm"><i class="ti ti-eye"></i></a>';
	...
	public function get_student_info($uniq_id)
	{
		$this->db->select('
			...
			c.bCertPSA, c.frm137, c.frm138,
			g.acad_yr,
			h.gender
		');
		$this->db->from('tbl_stud_info_kinder a');
		$this->db->join('tbl_stud_adtnl_info_kinder b', 'b.stud_lrn = a.stud_lrn');
		$this->db->join('tbl_stud_documents_kinder c', 'c.stud_lrn = a.stud_lrn');
	...
	public function get_discounts($id = FALSE)
	{
		if ($id === FALSE)
		{
			...
			$this->db->like('a.department', 'kinder', 'both');

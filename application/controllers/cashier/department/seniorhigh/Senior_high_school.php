...
class Senior_high_school extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model('cashier/seniorhs/Senior_high_school_model', 'shsdb');
	}
	...
	public function Index()
	{
		...
		$this->load->view('cashier/departments/seniorhs/index');
	...
	public function View_student($uniq_id)
	{
		...
		$this->load->view('templates/header');
		$this->load->view('templates/navigation');
		$this->load->view('cashier/departments/seniorhs/view_student', $data);
		$this->load->view('templates/footer');
	}
	...
	public function Get_payables_info()
	{
		if ($this->input->is_ajax_request())
		{
			$id = $this->input->get('id');
			$assessmentID = $this->input->get('assessmentID');
			$result = $this->shsdb->get_payables_details($id, $assessmentID);

...
class Discount_model extends CI_Model {
	public function get_discount_table($id = FALSE)
	{
		if ($id === FALSE)
		{
			$this->db->select('a.row_id, a.disc_code, a.discount, a.disc_amnt, a.status, b.dept_code');
	...
	public function check_disccode($dCode)
	{
		$this->db->select('disc_code');
		$this->db->from('tbl_discount');
		$this->db->where('disc_code', $dCode);
		$query = $this->db->get();
	...
	{
		$data = array(
			'disc_code' => $this->input->post('dcode'),
		...
		if ($this->db->insert('tbl_discount', $data))
		{
			$log = array(
				...
				'c_log' 	 => 'Created a new fee: '.$this->input->post('dcode').' - '.$this->input->post('dname').' - '.number_format($this->input->post('pcnt'), 2, '.', ',').' for '.$this->input->post('dFor'),
				'mod_date' => date('Y-m-d h:i:s a')
			);

			if ($this->db->insert('tbl_logs', $log))
			{
				$this->db->select('a.row_id, a.disc_code, a.discount, a.disc_amnt, a.disc_for, a.status, b.dept_code');
				$this->db->from('tbl_discount a');
				$this->db->join('tbl_departments b', 'b.dept_id = a.disc_for');
